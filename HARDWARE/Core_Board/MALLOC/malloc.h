...
#define SRAMCCM 2 //CCM内存池(此SRAM仅仅CPU可以访问!!!)

#define SRAMBANK 3 //定义支持的SRAM块数.

//mem1内存参数设定.mem1完全处于内部SRAM里面.
#define MEM1_BLOCK_SIZE 3									  //内存块大小为32字节
...
//mem3内存参数设定.mem3处于CCM,用于管理CCM(特别注意,这SRAM,仅CPU可以访问!!)
...
#define free(block) myfree(SRAMIN, block)
#define realloc(block, size) myrealloc(SRAMIN, block, size)
